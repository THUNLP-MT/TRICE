...
def lookup(inputs, mode, params, to_cpu=False):
    if mode != "infer":
        ...
        source, hypo, target = features["source"], features["hypo"], features["target"]
        source = source.numpy()
        hypo   = hypo.numpy()
        target = target.numpy()
        labels = labels.numpy()
        src_mask = torch.FloatTensor(features["source_mask"].numpy())
        hyp_mask = torch.FloatTensor(features["hypo_mask"].numpy())
        tgt_mask = torch.FloatTensor(features["target_mask"].numpy())

        if not to_cpu:
            src_mask = src_mask.cuda()
            tgt_mask = tgt_mask.cuda()
            hyp_mask = hyp_mask.cuda()

        source = _lookup(source, params.lookup["source"], to_cpu=to_cpu)
        hypo   = _lookup(hypo,   params.lookup["target"],   to_cpu=to_cpu)
    ...
    hypo   = inputs["hypo"].numpy()
    source = _lookup(source, params.lookup["source"], to_cpu=to_cpu)
    hypo   = _lookup(hypo,   params.lookup["target"],   to_cpu=to_cpu)
    src_mask = torch.FloatTensor(inputs["source_mask"].numpy())
    hyp_mask = torch.FloatTensor(inputs["hypo_mask"].numpy())
