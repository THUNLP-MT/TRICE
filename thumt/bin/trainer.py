...
def default_params():
    params = utils.HParams(
        ...
        clip_grad_norm=5.0,
        learning_rate=5e-5,
        special_learning_rate=5e-4,
        special_var_name="adapter",
...
def print_variables(model, pattern, log=True):
    ...
    for name in sorted(list(weights)):
        ...
        if log:
            print("%s %s requires_grad = %s" % (name.ljust(60), str(list(v.shape)).rjust(15), 'True' if v.requires_grad else 'False'))
...
def get_optimizer(params, schedule, clipper, special_schedule=None):
    if params.optimizer.lower() == "adam":
        optimizer = optimizers.AdamOptimizer(learning_rate=schedule,
                                             special_learning_rate=special_schedule,
                                             special_var_name=params.special_var_name,
...
def main(args):
    ...
    schedule = get_learning_rate_schedule(params, params.learning_rate)
    special_schedule = get_learning_rate_schedule(params, params.special_learning_rate)
    clipper = get_clipper(params)
    optimizer = get_optimizer(params, schedule, clipper, special_schedule=special_schedule)
