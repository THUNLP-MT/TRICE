...
def beam_search(models, features, params):
    ...
    bos_id = params.lookup["target"][params.tgt_lang_tok.encode("utf-8")]
    ...
    seq_length_hyp = features["hypo"].shape[1]
    features["hypo"] = torch.unsqueeze(features["hypo"], 1)
    features["hypo"] = features["hypo"].repeat([1, beam_size, 1])
    features["hypo"] = torch.reshape(features["hypo"],
                                       [batch_size * beam_size, seq_length_hyp])
    features["hypo_mask"] = torch.unsqueeze(features["hypo_mask"], 1)
    features["hypo_mask"] = features["hypo_mask"].repeat([1, beam_size, 1])
    features["hypo_mask"] = torch.reshape(features["hypo_mask"],
                                       [batch_size * beam_size, seq_length_hyp])
    ...
    init_seqs = torch.full([batch_size, beam_size, 1], bos_id, device=device, dtype=torch.int32)
